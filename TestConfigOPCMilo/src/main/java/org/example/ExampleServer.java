...
public class ExampleServer {
    ...
    static {
        ...
        //
        //Сначала код добавляет библиотеку BouncyCastleProvider, которая необходима для некоторых функций безопасности.

        Security.addProvider(new BouncyCastleProvider());

        try {
            //Затем он генерирует случайные числа, необходимые для безопасной работы.
            //Далее код создает временную директорию для хранения сертификатов безопасности сервера.
...
//        CompletableFuture — средство для передачи информации между параллельными потоками исполнения.
//        По существу это блокирующая очередь, способная передать только одно ссылочное значение.
//        В отличие от обычной очереди, передает также исключение,
//        если оно возникло при вычислении передаваемого значения.
